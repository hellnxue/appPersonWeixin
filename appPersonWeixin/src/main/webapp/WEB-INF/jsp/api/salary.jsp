...
<script >
    $(function () {
    	...
    	if(errorMessage){
    		$("#result").html('<div class="am-panel am-panel-danger"> <div class="am-panel-hd"> <h3 class="am-panel-title">错误消息</h3></div><div class="am-panel-bd">'+errorMessage+'</div></div>');
    		return;
    	}
    	
        $("#month").val($("#month").attr("month").replace(/(.{4})/g, '$1-'));
        $("#month").on('changeDate.datepicker.amui', function (event) {
            window.location.replace("${ctx}/api/salary?cardId=${enCardId}&month=" + getMonth(event.date));
