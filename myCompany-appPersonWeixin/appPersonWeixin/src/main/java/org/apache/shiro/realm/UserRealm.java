...
import com.service.provider.IJabavaCompanyProvider;
...
public class UserRealm extends AuthorizingRealm {
	...
	@Autowired
	private IJabavaCompanyProvider jabavaCompanyProvider;
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		if (user != null) {
			if (rememberMe) {
				...
				if (rememberLogin != null) {
					rememberLogin.setRemember(rememberMe);
					...
					rememberLogin.setOpenid(openId);
					rememberLogin.setRemember(true);
					rememberLoginService.save(rememberLogin);
					System.out.println("添加此用户");
				}
			}
		}
		...
		try {
			if (!"______".equals(password)) {
				...
				userS = centerUserService.loginAuth(CenterUserService.LOGIN_TYPE_LOGINNAME,
						CenterUserService.USER_TYPE_INDIVIDUAL, userName, password);
				if (userS != null && userS.getSuccess()) {
					...
					userGetInfo = centerUserService.getCenterUser(userName, CenterUserService.SYSTEM_ID_HF);
					userInfo = (CenterSysUser) userGetInfo.getResult();
					System.out.println("对应自动登录，根据登录验证取得的真正用户名获取用户信息。。。。。。。。。");
				...
				System.out.println("对应自动登录，直接根据用户名获取用户信息。。。。。。。。。");
		...
		if (userName != null && !"".equals(userName) && userGetInfo != null && userInfo != null) {
			...
			Long ucOrgId = userInfo.getOrgOrganization().getId();
			Map<String, Object> resultMap = new HashMap();
			Map<String, Object> map = new HashMap();
			resultMap.put("ucOrgId", ucOrgId);
			String mobile = (String) userInfo.getOrgPerson().getMobile();
			session.setAttribute("mobile", mobile);
			try {
				// 添加企业id
				map = jabavaCompanyProvider.getCompanyByUcOrgId(resultMap);
				Map resultData = (Map) map.get("resultData");
				Long companyId = null;
				String companyLogo = null;
				companyId = (Long) resultData.get("companyId");
				companyLogo = (String) resultData.get("companyLogo");
				session.setAttribute("companyLogo", companyLogo);
				session.setAttribute("companyId", companyId);
			} catch (Exception e) {
				System.out.println("该用户没有组织架构！");
			}
			// 处理自动登录
			if ("______".equals(password)) {
			...
			System.out.println("判断该登录用户。。。。。。。。。   没有自动登录");
			if (userS.getSuccess()) {
				System.out.println("判断该登录用户。。。。。。。。。   登陆成功");
				SimpleAuthenticationInfo sa = new SimpleAuthenticationInfo(userName, password, getName());
				// 获取微信用户名和用户类型
				openId = (String) session.getAttribute("UserName");
				apptype = (String) session.getAttribute("appType");
				System.out.println("判断该登录用户 openId    " + openId);
				if (openId == null) {
					return sa;
				}
				user = weChatService.findWeChatUser(openId, apptype);
				System.out.println("判断该登录用户。。。。。。。。。   根据openID获取用户+openId  " + openId + "apptype " + apptype);
				if (user == null) {
