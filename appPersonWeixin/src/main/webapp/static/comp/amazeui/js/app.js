...
function reloadUrl(ele){
  if($(ele).find(".am-icon-refresh")){
    $(ele).find(".am-icon-refresh").addClass(" am-icon-spin");
  }
...
$.table = {
  ...
  turnPage: function (pageSize, pn, child) {
    ...
    var prefix = $.table.getPrefix(table);
    pageURL = pageURL + prefix + "page=" + (pn-1);

    if (pageSize) {
      pageURL = pageURL + "&" + prefix + "size=" + pageSize;
    }
  ...
  removePageParam : function(pageURL) {
    ...
    pageURL = pageURL.replace(/\?\w*page.size=\d+\&/gi, '?');
    pageURL = pageURL.replace(/\?\w*page.size=\d+/gi, '');
    pageURL = pageURL.replace(/\&\w*page=\d+/gi, '');
    pageURL = pageURL.replace(/\&\w*size=\d+/gi, '');
    return pageURL;
  }
  ,
...
function getMonth(date) {
  ...
  return date.getFullYear().toString() + ( date.getMonth() >= 9 ? (date.getMonth() + 1).toString() : '0' + (date.getMonth() + 1));
}
