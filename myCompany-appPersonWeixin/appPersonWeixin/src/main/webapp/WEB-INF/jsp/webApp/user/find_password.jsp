			...
			<ul class="am-list m-widget-list"
				...
				<li>
					<div class="am-g lines">
						<span class="am-u-sm-8"><input value="" id="gCheckCode"
							class="am-form-field am-input-lg" name="gCheckCode"
							placeholder="请输入图片验证码" onblur="imgCode()"> </span> <span
							class="am-u-sm-4"> <img src="${ctx}/kaptcha.jpg"
							height="34" width="100" id="kaptchaImage" alt="图片验证码"
							title="看不清？点击更换验证码" />
						</span>
					</div>
				</li>
	...
	$(function() {
		...
		$("#sendcaptcha").on("click", function() {
			...
			var  msgcode = $("#gCheckCode").val();
	  	    $.getJSON("${ctx}/hrhelper-platform/anon/sendMessage.e", {
	           mobile: mobile, functionCode: "modifyMobile", msgcode:msgcode
			}, function (data) {
				if (data == true) {
		...
		$("#activeSubmit").on("click", function() {
			...
			$.getJSON("${ctx}/webApp/anon/duanxin", {
				...
				console.log(data);
				if (data) {
					$('#form1').attr('action', '${ctx}/webApp/anon/find_password');
					$('#form1').submit();
					return true;
				} else {
					$("#error").html("请填写验证码");
