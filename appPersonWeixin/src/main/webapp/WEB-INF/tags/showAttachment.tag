<%@ tag import="com.fnst.es.common.utils.ImagesUtils" %>
    ...
    <c:when test="${not empty filename}">
        <%
            ...
            if (showImage && ImagesUtils.isImage(filename)) {
                ...
                if (isCssSprite) {
                    template =
                            "<a href='%1$s/%2$s' target='_blank'><i style='display:inline-block;vertical-align: middle;background:url(%1$s/%2$s) -%3$spx -%4$spx no-repeat;height:%5$spx;width:%6$spx;%7$s'></i></a>";
                }
                if (isCssSprite) {
                ...
                template += "<a href='%s/%s' target='_blank'>" + "查看&nbsp;&nbsp;" + "</a>";
                template += "<a href='%s/%s' target='_blank'>" + "下载&nbsp;&nbsp;" + "</a>";
                out.write(String.format(template, ctx, newFileName, displayFileName, ctx, filename, ctx, newFileName));
            }
        %>
    </c:when>
    <c:otherwise>
