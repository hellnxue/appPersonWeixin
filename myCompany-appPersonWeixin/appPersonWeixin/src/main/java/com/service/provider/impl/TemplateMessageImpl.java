...
public class TemplateMessageImpl implements TemplateMessage {
	...
	private static final Logger logger = LoggerFactory.getLogger(TemplateMessageImpl.class);
	/**
	 * 发送模板消息
	 * 
	 * <pre>
	...
	public ReturnS sendTemplateMessage(String appType, String userName,
		...
		if (weChatUser == null || StringUtils.isBlank(weChatUser.getUsername())) {
			logger.error("获取微信用户失败,微信username  "+userName);
			returnS.setSuccess(false);
			returnS.setReturnCode("500");
		...
		if (resultType == null
			...
			logger.error("发送模板消息失败 没有返回失败原因  微信用户 "+userName);
			returnS.setSuccess(false);
		...
		if ("0".equals(resultType.getCode().toString().trim())) {
			...
			logger.error("发送模板消息失败 ,返回的code"+ resultType.getCode()+" 微信用户 "+userName);
			returnS.setSuccess(false);
	...
	private Map<String, Object> getMessageByTemplateCode(
		...
		if (TemplateCode.ORDER_STATUS_UPDATE.getValue().equals(templateCode)) {
		 ...
		 else if (TemplateCode.JIFEN_STATE_MESSAGE.getValue().equals(templateCode)) {// 积分状态提醒
			    templateId ="SbQ-8hX_WD22SYKNTTMjeJ6NXwAO7fet_5wKvsj3Crw";
				result.put("first",
						templateAPI.getParam1(data.get("first").toString()));
				result.put("account",
						templateAPI.getParam1(data.get("account").toString()));
				result.put("time",
						templateAPI.getParam1(data.get("time").toString()));
				result.put("type",
						templateAPI.getParam1(data.get("type").toString()));
				result.put("creditChange",
						templateAPI.getParam1(data.get("creditChange").toString()));
				result.put("number",
						templateAPI.getParam1(data.get("number").toString()));
				result.put("creditName",
						templateAPI.getParam1(data.get("creditName").toString()));
				result.put("amount",
						templateAPI.getParam1(data.get("amount").toString()));
				result.put("remark",
						templateAPI.getParam1(data.get("remark").toString()));
				return result;
			...
			if(data.get("remark")!=null && StringUtils.isNotBlank(data.get("remark").toString())){
				result.put("remark",
						templateAPI.getParam1(data.get("remark").toString()));
			}
